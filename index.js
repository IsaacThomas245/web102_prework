...
const displayStr = `A total of $${totalRaised.toLocaleString("en-US")} has been raised for ${GAMES_JSON.length.toLocaleString("en-US")} games. Currently, ${numUnfundedGames.toLocaleString("en-US")} ${numUnfundedGames === 1 ? "game remains unfunded" : "games remain unfunded"}. We need your help to fund these amazing games!`;
// create a new DOM element containing the template string and append it to the description container
const unfundedStrP = document.createElement("p");

unfundedStrP.innerHTML = `${displayStr}`;

descriptionContainer.appendChild(unfundedStrP);
...
/************************************************************************************
 * Bonus Feature: What if a user wants to use the nav bar at the top of the page to get to the Our Game section quickly?
 */
function scrollDownToGames() {
  const ourGamesSection = document.getElementById("our-games-section");

  ourGamesSection.scrollIntoView({ behavior: "smooth" });
}

const ourGamesButton = document.getElementById("our-games-button");
ourGamesButton.addEventListener("click", scrollDownToGames);
